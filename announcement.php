...
if(isset($_SESSION['eeid'])){
    ...
    if(isset($_GET['id'])){
        $announcement = get_announce_id($_GET['id']);
    }

    if(isset($_POST['upload_announcement'])){
        $name = $_POST['announce_name'];
        $desc = $_POST['announce_desc'];
        $image = $_FILES['announce_image'];

        insert_announcement($name,$desc,$image,$_SESSION['eeid'],$fullname);
    }
    ...
    if(isset($_POST['update'])){
        ...
        update_announcement($fullname,$_GET['id'],$name, $desc, $image);

    }
    if(isset($_POST['delete'])){
        delete_announcement($_POST['delete']);
    }
...
<head>
    ...
    <title>Chillaxr | Announcement</title>
                    ...
                    <div class="box-body">
                        <form role="form" method="POST" enctype="multipart/form-data">
                            <div class="input-group">
                                ...
                                <input type="text" name="announce_name" value="<?php if(isset($_GET['id'])){ echo $announcement['announcement_name']; }?>" class="form-control" placeholder="Announcement Name" required="required">
                            </div>
                            <br>

                            <div class="input-group">
                                <span class="input-group-addon"><i class="glyphicon glyphicon-comment"></i></span>
                                <input type="text" name="announce_desc" value="<?php if(isset($_GET['id'])){ echo $announcement['announcement_details']; }?>" class="form-control" placeholder="Description" required="required">
                            </div>

                            <br>
                            <div class="input-group">
                                <span class="input-group-addon"><i class="glyphicon glyphicon-picture"></i></span>
                                <input type="file" name="announce_image" value="<?php if(isset($_GET['id'])){ echo $announcement['announcement_img']; }?>" class="form-control" placeholder="Price" required="required">
                            </div>
                            <br>
                            <div>
                                <button type="submit" name="<?php
                                if(isset($_GET['id'])){ echo "update";}else{ echo "upload_announcement"; } ?>" class="btn btn-block btn-primary"><?php
                                    if(isset($_GET['id'])){ echo "Edit";}else{ echo "Save"; } ?></button>
                                <?php if(isset($_GET['id'])){?>
                                    <a href="announcement.php" class="btn btn-block btn-danger">Cancel</a>
                                <?php } ?>
                            </div>
                ...
                <div class="box bo  x-primary">
                    ...
                    <div class="box-body">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <?php
                            $announcement = get_all_announcement($_SESSION['eeid']);

                            foreach($announcement as $announce){

                                ?>
                                <tr>
                                    <td align="center"><img src="user_files/<?php echo $fullname;?>/announcement/photo/<?php echo $announce['announcement_img'];?>" width="50px" height="50px" align="center"></td>
                                    <td><?php echo $announce['announcement_name'];?></td>
                                    <td><?php echo $announce['announcement_details'];?></td>
                                    <td><?php echo $announce['announcement_date'];?></td>

                                    <td>

                                        <form role="form" method="post">
                                            <a class="btn btn-primary" href="announcement.php?id=<?php echo $announce['announcement_id'];?>"><span class="glyphicon glyphicon-edit"></span></a>

                                            <button class="btn btn-danger" type="submit" name="delete" value="<?php echo $announce['announcement_id'];?>"><span class="glyphicon glyphicon-trash"></span></button>
                                        </form>
