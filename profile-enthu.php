<?php
  ...
  if(isset($_SESSION['enthu_id'])){
    ...
    $otherenthuid = $_SESSION['enthu_id'];
...
<html lang="en">
  ...
  <div class="box box-success">
      ...
      <?php 
        $sql = "SELECT * FROM follow WHERE user=? AND status='1'";
        $stm = $con->prepare($sql);
        $stm->execute(array($otherenthuid));
        $userfollow = $stm->fetchall();
      ?>
      <div class="box-body chat" id="chat-box">
          <!-- chat item -->
          <?php
            foreach ($userfollow as $myfollow) {

          $sql = "SELECT * FROM entertainmentestablishment WHERE eeid=?";
          $stm = $con->prepare($sql);
          $stm->execute(array($myfollow['eeid']));
          $myannounce = $stm->fetch();
          $sql = "SELECT * FROM announcement WHERE eeid=?";
          $stm = $con->prepare($sql);
          $stm->execute(array($myannounce['eeid']));
          $myann = $stm->fetchAll();
          foreach($myann as $ann){
          ?>
          <div class="item">
              ...
              <p class="message">
                  <a href="#" class="name">
				  <br><br>
                      <?php echo $myannounce['establishment_name']; ?>
                      <br><br>
                  </a>
                  <?php
                  echo $ann['announcement_details'];?>
                    <br><img src="user_files/<?php echo $myannounce['establishment_name']; ?>/announcement/photo/<?php echo $ann['announcement_img']; ?>" height="200xp" width="200xp">
                  <?php }
                ?>
                </p>
            </div><!-- /.item -->
