...
if(isset($_SESSION['enthu_id'])){
    ...
    if(isset($_POST['reserveroom'])){
        ...
        //$range = $_POST['range'];
        $reservation_roomid = $_POST['reservationroomid'];
        $date= $_POST['date'];
        $eeid = $_POST['eeid'];
  ...
  /*      if(check_reservation($package,$range,$date, $time, $eeid)){
       ...
       /* else{*/
        ...
        insert_reservation($package,$hrrange,$date,$datetime,$_SESSION['enthu_id'],$eeid,$time,$reservation_roomid);
...
<?php
    if(isset($_GET['id'])){
        include('estab-view.php');

    }else{
        include('estab-list.php');
    }
?>
...
<footer class="main-footer">
    <div class="container">
        ...
        <strong>Copyright &copy; 2014-2016 <a href="index.php">Chillaxr</a>.</strong> All rights reserved.
    </div><!-- /.container -->
</footer>
</div><!-- ./wrapper -->
            ...
            onSelect: function (dateText, inst){
                ...
                $.get('reservationChecker.php?eeid='+ $('#eeid').val(), function(data){
                    ...
                    for(i=0;i<data.length;i++){
                        ...
                        else if(String(data[i].reservation_date) == String(selectedDate) && $('#packageid').val() == 0){
                            $('input#'+data[i].attr_tr_range).attr('type', 'hidden');
                            //alert('ssss');
                        }
                    }
                });
                $.getJSON('reservationCheckerNo.php?eeid='+$('#eeid').val(),function(data){

                  $.each(data,function(value,result){
                    if(String(result.reservation_date) == String(selectedDate) && $('#packageid').val() == 0){
                        $('input#'+result.attr_tr_range).attr('type', 'hidden');
                        //alert('ssss');
                    }
                  });
                });
